...
li_repo_entry_init (LiRepoEntry *re)
{
	...
	priv->url = NULL;
	priv->kinds = LI_REPO_INDEX_KIND_NONE;
}
...
li_repo_entry_parse (LiRepoEntry *re, const gchar *repo_line)
{
	...
	for (i = 0; kind_parts[i] != NULL; i++) {
		...
		kind = li_repo_index_kind_from_string (kind_parts[i]);
...
li_repo_entry_get_index_urls_for_arch (LiRepoEntry *re, const gchar *arch)
{
	...
	if (li_repo_entry_has_kind (re, LI_REPO_INDEX_KIND_NONE))
		return NULL;

	urls = g_ptr_array_new_with_free_func (g_free);

	if (li_repo_entry_has_kind (re, LI_REPO_INDEX_KIND_COMMON))
		g_ptr_array_add (urls, g_build_filename (priv->url, "indices", arch, "Index.gz", NULL));

	if (li_repo_entry_has_kind (re, LI_REPO_INDEX_KIND_DEVEL))
		g_ptr_array_add (urls, g_build_filename (priv->url, "indices", arch, "Index-Devel.gz", NULL));

	if (li_repo_entry_has_kind (re, LI_REPO_INDEX_KIND_SOURCE))
 ...
 */
void
li_repo_entry_set_kinds (LiRepoEntry *re, LiRepoIndexKinds kinds)
{
	LiRepoEntryPrivate *priv = GET_PRIVATE (re);
	priv->kinds = kinds;
}
 ...
 */
void
li_repo_entry_add_kind (LiRepoEntry *re, LiRepoIndexKinds kind)
{
	LiRepoEntryPrivate *priv = GET_PRIVATE (re);
	priv->kinds |= kind;
}
 ...
 */
gboolean
li_repo_entry_has_kind (LiRepoEntry *re, LiRepoIndexKinds kind)
{
	LiRepoEntryPrivate *priv = GET_PRIVATE (re);
...
li_repo_index_kind_to_string (LiRepoIndexKinds kind)
{
	...
	if (kind == LI_REPO_INDEX_KIND_COMMON)
		return "common";
	if (kind == LI_REPO_INDEX_KIND_DEVEL)
		return "devel";
	if (kind == LI_REPO_INDEX_KIND_SOURCE)
		return "source";
	return NULL;
}
 ...
 *
 * Returns: a %LiRepoIndexKinds or %LI_REPO_INDEX_KIND_NONE for unknown
 **/
LiRepoIndexKinds
li_repo_index_kind_from_string (const gchar *kind_str)
{
	if (g_strcmp0 (kind_str, "common") == 0)
		return LI_REPO_INDEX_KIND_COMMON;
	if (g_strcmp0 (kind_str, "devel") == 0)
		return LI_REPO_INDEX_KIND_DEVEL;
	if (g_strcmp0 (kind_str, "source") == 0)
		return LI_REPO_INDEX_KIND_SOURCE;
	return LI_REPO_INDEX_KIND_NONE;
}
